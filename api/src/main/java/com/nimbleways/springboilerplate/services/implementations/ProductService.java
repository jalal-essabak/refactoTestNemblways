...
public class ProductService {
    ...
    public void handleFlashSaleProduct(Product p){
        if(checkLimitPeriodeForFlashSalesProduct(p) && p.getFlashSaleLimitOrder() > 0){
            p.setFlashSaleLimitOrder(p.getFlashSaleLimitOrder() -1);
            pr.save(p);
        }else{
            ns.sendOutOfStockNotification(p.getName());
            p.setFlashSaleLimitOrder(0);
            pr.save(p);
        }
    }

    public boolean checkLimitPeriodeForFlashSalesProduct(Product p){
        return p.getFlashSaleStartDate().isBefore(LocalDate.now()) && p.getFlashSaleEndDate().isAfter(LocalDate.now());
    }
